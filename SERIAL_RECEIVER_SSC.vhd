...
End SERIAL_RECEIVER_SSC;

Architecture structural of SERIAL_RECEIVER_SSC is
...
U2 : PARITY_CHECK   port map( data => SDX, init => carry_init, CLK => SCLK, err => carry_err);
U3 : COUNTER_UP	  port map( dataIn => "0000", RESET => carry_init, PL => '0', CE => '1', CLK => SCLK, Q => carry_count);
U4 : EQUAL_9		  port map( D => carry_count, Y => EQUALS_9);
U5 : EQUAL_10		  port map( D => carry_count, Y => EQUALS_10);
U6 : SHIFT_REG_7	  port map( D_7bit => "0000000", PL => '1', data => SDX, EN => carry_wr, clk => SCLK, Clear => '0', D => D);
